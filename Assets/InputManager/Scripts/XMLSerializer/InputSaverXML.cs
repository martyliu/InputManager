...
public class InputSaverXML : IInputSaver
{
    ...
    public void Save(List<KeyboardMouseControlScheme> keymouseSchemes, List<JoystickControlScheme> joystickSchemes)
    {
        ...
        using (XmlWriter writer = CreateXmlWriter(xmlSettings))
        {
            ...
            foreach(var scheme in keymouseSchemes)
                scheme.SerializeToXml(writer);
            foreach(var scheme in joystickSchemes)
                scheme.SerializeToXml(writer);
